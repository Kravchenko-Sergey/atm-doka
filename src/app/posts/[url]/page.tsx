...
const DevicePage = () => {
	...
	const post = useMemo(() => posts.find(post => post.url === url), [posts, url])

	const postIndex = useMemo(() => posts.findIndex(p => p.id === post?.id), [posts, post?.id])

	const prevPost = useMemo(() => posts[postIndex - 1] ?? null, [posts, postIndex])

	const nextPost = useMemo(() => posts[postIndex + 1] ?? null, [posts, postIndex])
	...
	return (
		<div className='w-full flex flex-col'>
			...
			<div className='flex flex-col items-center'>
				<header className='flex items-center justify-center w-screen h-[480px] z-0 md:h-[348px]' style={{ backgroundColor: post.bgColor }}>
					<div className='w-full px-4 max-w-[1308px] flex flex-col items-center gap-6 md:flex-row md:gap-8'>
						<Image src={post.image} width={320} height={320} priority alt={post.title} className='w-auto h-[320px] object-contain' />
						<div className='flex flex-col gap-4 text-center md:text-left'>
							<h1 className='text-3xl font-semibold'>{post.title}</h1>
							<p className='text-xl text-gray-700 dark:text-gray-300'>{post.description}</p>
						</div>
					</div>
				</header>
			...
			<div className='flex flex-col lg:flex-row'>
				...
				<main className='flex-1 max-w-full lg:max-w-[calc(100%-280px)]'>
					...
					<div className='px-4 max-w-[1308px] mx-auto'>
						<h2 className='my-12 text-3xl font-semibold transition-colors duration-300' style={getActiveStyles(String(post.contentItems.length))}>
							Читайте также
						</h2>
						<div className='flex gap-6 flex-wrap'>
							{randomPosts.map(post => (
								<Link href={`/posts/${post.url}`} key={post.id} className={`relative p-6 h-[380px] flex flex-col items-center justify-between rounded-xl overflow-hidden transition-all duration-300 ease-in-out hover:shadow-lg hover:scale-101 flex-1 min-w-[280px] ${'max-w-full'}`} style={{ backgroundColor: post.bgColor }}>
									...
									<h3 className='text-xl font-semibold self-start z-10 line-clamp-2'>{post.title}</h3>
									<p className='text-lg text-center self-center z-10 line-clamp-3'>{post.description}</p>
						...
						<div className='my-12 flex flex-col gap-6 md:flex-row justify-between items-center'>
							{prevPost && (
								<Link href={`/posts/${prevPost.url}`} className='flex items-center gap-3 text-lg hover:underline transition-all hover:scale-101 group mr-auto'>
									<CircleArrowLeft className='transition-transform group-hover:-translate-x-1' />
									<span className='max-w-[200px] line-clamp-2 md:max-w-[300px]'>{prevPost.title}</span>
								</Link>
							)}

							{nextPost && (
								<Link href={`/posts/${nextPost.url}`} className='flex items-center gap-3 text-lg hover:underline transition-all hover:scale-101 group ml-auto'>
									<span className='max-w-[200px] line-clamp-2 md:max-w-[300px] text-right'>{nextPost.title}</span>
									<CircleArrowRight className='transition-transform group-hover:translate-x-1' />
								</Link>
							)}
						</div>

						<ScrollToTop progressColor={post.bgColor} iconColor={post.bgColor} />
