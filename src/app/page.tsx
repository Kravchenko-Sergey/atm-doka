...
export default function Home() {
	...
	const filteredPosts = useMemo(() => {
		...
		return sortedPosts.filter(post => activeTags.some(activeTag => post.tags.includes(activeTag)))
	}, [sortedPosts, activeTags])

	const displayedPosts = useMemo(() => filteredPosts.slice(0, visibleCount), [filteredPosts, visibleCount])
			...
			<div className='pt-20 md:pt-4 px-4 max-w-[1572px] text-xl flex flex-wrap gap-4 pb-10'>
				...
				${tag.isActive ? '' : 'border-gray-300 dark:border-gray-600'}`}
						style={{
							...
							borderRadius: '0.5rem',
							backgroundColor: tag.isActive ? `${tag.borderColor}20` : 'transparent'
						}}>
						{tag.title}
					</button>
				))}
				...
				<div className='flex flex-col gap-8'>
					<div className='flex justify-center gap-6 flex-wrap'>
						{displayedPosts.map(post => (
							<Link href={`/posts/${post.url}`} key={post.id} className='relative p-6 min-w-[320px] h-[330px] flex flex-col items-center justify-between flex-1 rounded-xl overflow-hidden transition-all duration-300 ease-in-out sm:h-[480px] sm:min-w-[400px] hover:shadow-lg hover:scale-101' style={{ backgroundColor: post.bgColor }}>
								...
								<h3 className='text-2xl font-semibold self-start z-10'>{post.title}</h3>
								<p className='text-xl text-center self-center z-10 line-clamp-3'>{post.description}</p>
								<div className='flex gap-3 flex-wrap self-end z-10'>
									{post.tags.map(tag => (
										<span key={tag} className='px-2 py-1 text-sm bg-black/10 rounded-md'>
											{tag}
										</span>
									))}
								</div>
					...
					{filteredPosts.length > visibleCount && (
						<div className='flex justify-center'>
							<Button onClick={handleLoadMore} variant='outline' className='p-6 text-xl rounded-xl font-normal min-w-[200px] hover:scale-105 transition-transform'>
								Показать ещё
							</Button>
						</div>
					)}
